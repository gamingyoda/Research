...
static void MUSCL_calc(int r0, int dK, int n, MUSCL_struct *st){
  ...
  double D1rho, D1ux, D1uy, D1e, D1p, D1k, D1omega;
  double D2rho, D2ux, D2uy, D2e, D2p, D2k, D2omega;
  double D3rho, D3ux, D3uy, D3e, D3p, D3k, D3omega;
  double D1rho_, D1ux_, D1uy_, D1p_, D1e_, D1k_, D1omega_;
  double D2rho_, D2ux_, D2uy_, D2p_, D2e_, D2k_, D2omega_;
  double D3rho_, D3ux_, D3uy_, D3p_, D3e_, D3k_, D3omega_;
  double Jaco;
  ...
  D2e   = e[n][r0]   - e[n][mK];
  D3e   = e[n][pK]   - e[n][r0];
  D1k       = k_turb[n][mK]  - k_turb[n][mmK];
  D2k       = k_turb[n][r0]  - k_turb[n][mK];
  D3k       = k_turb[n][pK]  - k_turb[n][r0];
  D1omega   = omega[n][mK]   - omega[n][mmK];
  D2omega   = omega[n][r0]   - omega[n][mK];
  D3omega   = omega[n][pK]   - omega[n][r0];
  ...
  st->kPK  = k_turb[n][mK] + 0.25 * ss * ((1.0 - kk * ss) * D1k_ + (1.0 + kk * ss) * D2k_) * ipu;

  D1omega_ = minmod_limiter(D1omega, D2omega, kk);
  D2omega_ = minmod_limiter(D2omega, D1omega, kk);
  st->omegaPK  = omega[n][mK] + 0.25 * ss * ((1.0 - kk * ss) * D1omega_ + (1.0 + kk * ss) * D2omega_) * ipu;
  ...
  st->kMK  = k_turb[n][r0] - 0.25 * ss * ((1.0 - kk * ss) * D3k_ + (1.0 + kk * ss) * D2k_ ) * ipu;

  D3omega_ = minmod_limiter(D3omega, D2omega, kk);
  D2omega_ = minmod_limiter(D2omega, D3omega, kk);
  st->omegaMK  = omega[n][r0] - 0.25 * ss * ((1.0 - kk * ss) * D3omega_ + (1.0 + kk * ss) * D2omega_ ) * ipu;

  st->pMK = (gamma0 - 1.0) * ( st->eMK - 0.5 * st->rhoMK * (st->uxMK*st->uxMK + st->uyMK*st->uyMK));
  st->cMK   = sqrt( gamma0*st->pMK/st->rhoMK );/** 音速 **/
}

static void slau_core(int r0,int n,double EFG[2][6]){
  int m;
  double Cbar, Mbar, ML, MR, betaL, betaR, pbar;
  double ukPK, ukMK, ukbarR, ukbarL, ukbar, CC, CL, CR, CCL, CCR, HHL, HHR;
  double utPK, utMK;
  double s0, s1;
  double mom;
  double uPK,uMK;
  double Xuai, Gmix, Jaco;
  double s2, s3, Sk;
  MUSCL_struct st;
  ...
  for(m=0; m<2; m++){
    ...
    if(m == 0){ /* E */
      ...
      s1 = x_eta[n][r0] * Jaco;
      Sk = sqrt(s0 * s0 + s1 * s1);
      ukPK = (st.uxPK * s0 + st.uyPK * s1) / sqrt(s0 * s0 + s1 * s1);
      ukMK = (st.uxMK * s0 + st.uyMK * s1) / sqrt(s0 * s0 + s1 * s1);
      /* s0 = y_eta[n][r0]; */
      /* s1 = x_eta[n][r0]; */
      /* Sk = sqrt(s0 * s0 + s1 * s1); */
      /* ukPK = (st.uxPK * s0 + st.uyPK * s1); */
      /* ukMK = (st.uxMK * s0 + st.uyMK * s1); */
      /* utPK = (-st.uxPK * s1 + st.uyPK * s0) / sqrt(s0 * s0 + s1 * s1); */
      /* utMK = (-st.uxMK * s1 + st.uyMK * s0) / sqrt(s0 * s0 + s1 * s1); */
    }
    else if(m == 1){ /* F */
    ...
    /** 音速の平均値を求める **/ 
    CCL  = sqrt(2.0 * (gamma0 - 1.0) * (st.ePK + st.pPK) / (st.rhoPK * (gamma0 + 1.0)));
    CCR  = sqrt(2.0 * (gamma0 - 1.0) * (st.eMK + st.pMK) / (st.rhoMK * (gamma0 + 1.0)));
    CL   = CCL * CCL / max2f(CCL, fabs(ukPK));
    CR   = CCR * CCR / max2f(CCR, fabs(ukMK));
    Cbar = min2f(CL, CR);
    ...
    /* Mbar = min2f(1.0, sqrt(0.5 * (ukPK * ukPK + ukMK * ukMK)) / Cbar); */
    ...
    pbar = 0.5 * (st.pPK + st.pMK) + 0.5 * (betaL - betaR) * (st.pPK - st.pMK) + 0.5 * sqrt(0.5 * (st.uxPK * st.uxPK + st.uxMK * st.uxMK + st.uyPK * st.uyPK + st.uyMK * st.uyMK))  * (betaL + betaR - 1.0) * (st.rhoPK + st.rhoMK) * Cbar;
    ...
    EFG[m][4] = Sk * (0.5*(mom+fabs(mom))*st.kPK                     + 0.5*(mom-fabs(mom))*st.kMK) / Jaco;
    EFG[m][5] = Sk * (0.5*(mom+fabs(mom))*st.omegaPK                 + 0.5*(mom-fabs(mom))*st.omegaMK) / Jaco;
    /* EFG[m][0] = (0.5*(mom+fabs(mom))                            + 0.5*(mom-fabs(mom))) / Jaco; */
    /* EFG[m][1] = (0.5*(mom+fabs(mom))*    st.uxPK                + 0.5*(mom-fabs(mom))*    st.uxMK        + s0*pbar) / Jaco; */
    /* EFG[m][2] = (0.5*(mom+fabs(mom))*    st.uyPK                + 0.5*(mom-fabs(mom))*    st.uyMK        + s1*pbar) / Jaco; */
    /* EFG[m][3] = (0.5*(mom+fabs(mom))*(st.ePK+st.pPK)/st.rhoPK   + 0.5*(mom-fabs(mom))*(st.eMK+st.pMK)/st.rhoMK) / Jaco; */
    /* EFG[m][4] = (0.5*(mom+fabs(mom))*st.kPK                     + 0.5*(mom-fabs(mom))*st.kMK) / Jaco; */
    /* EFG[m][5] = (0.5*(mom+fabs(mom))*st.omegaPK                 + 0.5*(mom-fabs(mom))*st.omegaMK) / Jaco; */
    /* EFG[m][0] = (0.5*(mom+fabs(mom))                            + 0.5*(mom-fabs(mom))); */
    /* EFG[m][1] = (0.5*(mom+fabs(mom))*    st.uxPK                + 0.5*(mom-fabs(mom))*    st.uxMK        + s0*pbar); */
    /* EFG[m][2] = (0.5*(mom+fabs(mom))*    st.uyPK                + 0.5*(mom-fabs(mom))*    st.uyMK        + s1*pbar); */
    /* EFG[m][3] = (0.5*(mom+fabs(mom))*(st.ePK+st.pPK)/st.rhoPK   + 0.5*(mom-fabs(mom))*(st.eMK+st.pMK)/st.rhoMK); */
    /* EFG[m][4] = (0.5*(mom+fabs(mom))*st.kPK                     + 0.5*(mom-fabs(mom))*st.kMK); */
    /* EFG[m][5] = (0.5*(mom+fabs(mom))*st.omegaPK                 + 0.5*(mom-fabs(mom))*st.omegaMK); */

    /* if(m==0){ */
    /*   EFG[m][0] = (0.5*(mom+fabs(mom))                            + 0.5*(mom-fabs(mom))) / Jaco; */
    /*   EFG[m][1] = (0.5*(mom+fabs(mom))* (s0 * ukPK - s1 * utPK)       */
    /* 	         + 0.5*(mom-fabs(mom))* (s0 * ukMK - s1 * utMK)  + s0*pbar) / Jaco; */
    /*   EFG[m][2] = (0.5*(mom+fabs(mom))* (s1 * ukPK + s0 * utPK)       */
    /* 	         + 0.5*(mom-fabs(mom))* (s1 * ukMK + s0 * utMK)  + s1*pbar) / Jaco; */
    /*   EFG[m][3] = (0.5*(mom+fabs(mom))*(st.ePK+st.pPK)/st.rhoPK   + 0.5*(mom-fabs(mom))*(st.eMK+st.pMK)/st.rhoMK) / Jaco; */
    /*   EFG[m][4] = (0.5*(mom+fabs(mom))*(st.ePK+st.pPK)/st.rhoPK   + 0.5*(mom-fabs(mom))*(st.eMK+st.pMK)/st.rhoMK) / Jaco; */
    /*   EFG[m][5] = (0.5*(mom+fabs(mom))*(st.ePK+st.pPK)/st.rhoPK   + 0.5*(mom-fabs(mom))*(st.eMK+st.pMK)/st.rhoMK) / Jaco; */
    /* } */
    /* else if(m==1){ */
    /*   EFG[m][0] = (0.5*(mom+fabs(mom))                            + 0.5*(mom-fabs(mom))) / Jaco; */
    /*   EFG[m][1] = (0.5*(mom+fabs(mom))* (s0 * ukPK + s1 * utPK)       */
...
#pragma omp parallel for private(i, j)
	...
	E[n][2][r0]     = EFG[_E_][2];
	E[n][3][r0]     = EFG[_E_][3];
	TurbE[n][0][r0] = EFG[_E_][4];
	TurbE[n][1][r0] = EFG[_E_][5];

	F[n][0][r0]     = EFG[_F_][0];
	F[n][1][r0]     = EFG[_F_][1];
	F[n][2][r0]     = EFG[_F_][2];
	F[n][3][r0]     = EFG[_F_][3];
	TurbF[n][0][r0] = EFG[_F_][4];
	TurbF[n][1][r0] = EFG[_F_][5];
